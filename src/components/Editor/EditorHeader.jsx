  ...
  return (
    <header className={`border-b p-3 ${isLight ? 'border-slate-300' : 'border-slate-700'}`}>
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0 overflow-hidden">
          ...
          {breadcrumbs.map((crumb, index) => (
            <span key={index} className={`text-sm truncate ${isLight ? 'text-slate-600' : 'text-slate-400'}`}>
              {crumb.replace('.md', '')}
              {index < breadcrumbs.length - 1 && <ChevronRight className="w-3 h-3 inline mx-1 shrink-0" />}
            </span>
          ))}
        </div>
        ...
        {/* Editor Mode */}
        <span className={`${isLight ? 'text-slate-400' : 'text-slate-500'} font-mono whitespace-nowrap hidden md:block shrink-0`}>
